 ...
 */
definition(
    name: "Trigger Alert Mode",
    namespace: "claytonjn",
    author: "claytonjn",
    description: "Changes mode to Alert if one of the \"Alert\" virtual switches is on.",
    category: "My Apps",
...
preferences {
	page(name: "page", install: true, uninstall: true) {
        section("Preferences") {
            paragraph "Changes mode to Alert if one of the \"Alert\" virtual switches is on."
            input "switches", "capability.switch", title: "\"Alert\" Switch(s)", multiple: true
        }
    }
}
...
def initialize() {
    subscribe(settings.switches, "switch.on", switchOnHandler)
}

void switchOnHandler(evt) {
	location.setMode("Alert")
    sendNotificationEvent("Mode changed to Alert!")
}